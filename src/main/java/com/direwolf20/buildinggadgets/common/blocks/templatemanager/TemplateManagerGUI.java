...
public class TemplateManagerGUI extends GuiContainer {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (buttonHelp.isSelected()) {
            ...
            for (IHoverHelpText helpTextProvider : helpTextProviders) {
                if (helpTextProvider.isHovered(mouseX, mouseY)) drawHoveringText(
    ...
    private GuiButton createAndAddButton(int id, int x, int y, int witdth, int height, String text) {
        GuiButtonHelpText button = new GuiButtonHelpText(
            id,
            this.guiLeft + x,
            this.guiTop + y,
            witdth,
    ...
    public void drawTexturedModalRectReverseX(int x, int y, int textureX, int textureY, int width, int height,
        boolean reverse) {
        ...
        if (reverse) {
            tessellator.addVertexWithUV(
                ...
                y + height,
                zLevel,
                (textureX + width) * GuiUtils.TEXTURE_CONVERSION_FACTOR,
                textureY * GuiUtils.TEXTURE_CONVERSION_FACTOR);
            tessellator.addVertexWithUV(
                x + width,
                y + height,
                zLevel,
                textureX * GuiUtils.TEXTURE_CONVERSION_FACTOR,
                textureY * GuiUtils.TEXTURE_CONVERSION_FACTOR);
            tessellator.addVertexWithUV(
                x + width,
                y + 0,
                zLevel,
                textureX * GuiUtils.TEXTURE_CONVERSION_FACTOR,
                (textureY + height) * GuiUtils.TEXTURE_CONVERSION_FACTOR);
            tessellator.addVertexWithUV(
                x + 0,
                y + 0,
                zLevel,
    ...
    // drawRect(guiLeft + panel.getX() - 1, guiTop + panel.getY() - 1, guiLeft + panel.getX() + panel.getWidth() + 1,
    // guiTop + panel.getY() + panel.getHeight() + 1, 0xFF8A8A8A);
    ...
    // GL11.glViewport((guiLeft + panel.getX()) * scale, mc.displayHeight - (guiTop + panel.getY() + panel.getHeight())
    // * scale, panel.getWidth() * scale, panel.getHeight() * scale);
    ...
    protected void actionPerformed(GuiButton b) {
        if (b.id == buttonHelp.id) {
            ...
            PacketHandler.INSTANCE
                .sendToServer(new PacketTemplateManagerLoad(new ChunkCoordinates(te.xCoord, te.yCoord, te.zCoord)));
        } else if (b.id == 2) {
    ...
    protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        ...
        if (!nameField.isFocused() && nameField.getText()
            .isEmpty())
            fontRendererObj.drawString(
                "template name",
                nameField.xPosition - guiLeft + 4,
                nameField.yPosition - guiTop,
                -10197916);

        if (GuiUtils.isHovered(buttonSave, mouseX, mouseY) || GuiUtils.isHovered(buttonLoad, mouseX, mouseY)
            || GuiUtils.isHovered(buttonPaste, mouseX, mouseY))
            drawSlotOverlay(
                GuiUtils.isHovered(buttonLoad, mouseX, mouseY) ? container.getSlot(0) : container.getSlot(1));
    }
