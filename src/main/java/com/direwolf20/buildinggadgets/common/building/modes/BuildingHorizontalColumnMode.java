...
public class BuildingHorizontalColumnMode extends AtopSupportedMode {
    ...
    public ChunkCoordinates transformAtop(EntityPlayer player, ChunkCoordinates hit, EnumFacing sideHit,
        ItemStack tool) {
        return VectorTools.isAxisVertical(sideHit)
            ? ChunkCoordinateUtils.offset(hit, VectorTools.getHorizontalFacingFromPlayer(player))
            : ChunkCoordinateUtils.offset(hit, DirectionUtils.getOppositeEnumFacing(sideHit));
    }
