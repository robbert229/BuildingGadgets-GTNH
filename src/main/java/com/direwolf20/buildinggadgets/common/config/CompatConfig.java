...
public class CompatConfig {
    ...
    public static boolean readConfig(File compatConfigFile) {
        if (compatConfigFile == null || !compatConfigFile.exists()) return false;
        ...
        try {
            ...
            BuildingGadgets.LOG.error("Error loading compat-config file! ", e1);
        }
        return false;
    }
    ...
    private static void parseCompatConfig(Configuration cfg) {
        ...
        energyCostExchanger = cfg.getInt(
            "energyCostExchanger",
            CATEGORY_GENERAL,
            energyCostExchanger,
            0,
        ...
        energyCostDestruction = cfg.getInt(
            "energyCostDestruction",
            CATEGORY_GENERAL,
            energyCostDestruction,
            0,
            100000,
            "The energy cost of the Destruction Gadget per block");
        energyMax = cfg.getInt(
            "energyMax",
            CATEGORY_GENERAL,
            energyMax,
            0,
            Integer.MAX_VALUE,
            "The max energy of the Builder & Exchanger");
        energyMaxDestruction = cfg.getInt(
            "energyMaxDestruction",
            CATEGORY_GENERAL,
            energyMaxDestruction,
            0,
            Integer.MAX_VALUE,
            "The max energy of the Destruction Gadget");
        poweredByFE = cfg.getBoolean(
            "poweredByFE",
            CATEGORY_GENERAL,
            poweredByFE,
            "Set to true for Forge Energy Support, set to False for vanilla Item Damage");
        durabilityBuilder = cfg.getInt(
            "durabilityBuilder",
            CATEGORY_GENERAL,
            durabilityBuilder,
            0,
        ...
        durabilityExchanger = cfg.getInt(
            "durabilityExchanger",
            CATEGORY_GENERAL,
            durabilityExchanger,
            0,
        ...
        durabilityDestruction = cfg.getInt(
            "durabilityDestruction",
            CATEGORY_GENERAL,
            durabilityDestruction,
            0,
            100000,
            "The max durability of the Destruction Gadget (Ignored if powered by FE)");
        durabilityCopyPaste = cfg.getInt(
            "durabilityCopyPaste",
            CATEGORY_GENERAL,
            durabilityCopyPaste,
            0,
        ...
        enablePaste = cfg.getBoolean(
            "enablePaste",
            CATEGORY_GENERAL,
            enablePaste,
            "Set to false to disable the recipe for construction paste.");
        enableDestructionTool = cfg.getBoolean(
            "enableDestructionTool",
            CATEGORY_GENERAL,
            enableDestructionTool,
            "Set to false to disable the destruction tool.");
        absoluteCoordDefault = cfg.getBoolean(
            "absoluteCoordinateModeDefault",
            CATEGORY_GENERAL,
            absoluteCoordDefault,
            "Determines if the Copy/Paste GUI's coordinate mode starts in 'Absolute' mode by default. Set to true for Absolute, set to False for Relative.");
        allowAbsoluteCoords = cfg.getBoolean(
            "allowAbsoluteCoords",
            CATEGORY_GENERAL,
            allowAbsoluteCoords,
            "Disabled absolute coords mode for the copy paste gadget?");
        canOverwriteBlocks = cfg.getBoolean(
            "canOverwriteBlocks",
            CATEGORY_GENERAL,
            canOverwriteBlocks,
            "Whether the Builder / CopyPaste gadgets can overwrite blocks like water, lava, grass, etc (like a player can). False will only allow it to overwrite air blocks.");
        ...
        if (!category.isEmpty()) blacklist = category.get("Blacklist")
            .getStringList();
    }
