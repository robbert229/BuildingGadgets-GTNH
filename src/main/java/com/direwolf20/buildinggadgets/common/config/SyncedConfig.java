...
public class SyncedConfig {
    ...
    public static void sendConfigUpdateTo(EntityPlayerMP player) {
        if (FMLCommonHandler.instance()
            ...
            // Testing showed, that this does absolutely nothing if called from outside game (besides taking up a small
            // bit of processing time of course)
    ...
    public static void onReadSynchronisation(NBTTagCompound compound) {
        ...
        transferValues();
        if (!compound.hasKey(KEY_VALUE)) return;
        ...
        for (int i = 0; i < list.tagCount(); i++) {
            NBTBase rawNBT = list.getCompoundTagAt(i);
            if (rawNBT instanceof NBTTagCompound) {
                handleNBT((NBTTagCompound) rawNBT, map);
            } else {
                BuildingGadgets.LOG.warn(
                    "Unexpected " + rawNBT.getClass()
                        .getName() + " found in NBTTagList which was expected to only contain NBTTagCompounds!");
            }
        }
    }

    private static String getMapperIdFor(Field f) {
        return f.getAnnotation(AutoSync.class)
            .mapperId();
    }
    ...
    private static NBTTagCompound parseField(Field field) {
        ...
        if (valueTag == null) {
            BuildingGadgets.LOG.warn(
                "Could not use type of Field " + field.getName()
                    + "!"
                    + " Found type "
                    + field.getType()
                        .getName()
    ...
    private static List<Field> getSyncFields() {
        return ReflectionTool.getFilteredFields(
            SyncedConfig.class,
            field -> field.isAnnotationPresent(AutoSync.class) && ReflectionTool.PREDICATE_STATIC.test(field));
    }

    private static String getSyncName(Field field) {
        String name = field.getAnnotation(AutoSync.class)
            .value();
