 ...
 * The value given to {@link #mapToField(Object)} will be the deserialized Value from an appropriate
 * {@link ITypeSerializer}.
 * 
...
public class FieldMapper<FieldVal, SyncedVal> {
     ...
     * (list) -> ((ImmutableList<Block>)list).stream().map((b) ->
     * Objects.requireNonNull(b.getRegistryName()).toString()).toArray(String[]::new),
     * (strings) ->
     * Stream.of(strings).map(ResourceLocation::new).map(ForgeRegistries.BLOCKS::getValue).collect(ImmutableList.
     * toImmutableList()),
...
//        String[].class);
    ...
    public static <FieldVal, SyncedVal> FieldMapper<FieldVal, SyncedVal> of(Function<FieldVal, SyncedVal> fieldToSync,
        Function<SyncedVal, FieldVal> syncToField, Class<FieldVal> fieldType, Class<SyncedVal> syncedType) {
    ...
    private FieldMapper(Function<FieldVal, SyncedVal> fieldToSync, Function<SyncedVal, FieldVal> syncToField,
        Class<FieldVal> fieldType, Class<SyncedVal> syncedType) {
