...
public class GadgetBuilding extends GadgetGeneric {
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List<String> list, boolean b) {
        ...
        list.add(
            ChatFormatting.AQUA + StatCollector.translateToLocal("tooltip.gadget.mode")
                + ": "
                + (mode == BuildingModes.Surface && getConnectedArea(stack)
        ...
        if (getToolMode(stack) != BuildingModes.BuildToMe) {
            list.add(
                ChatFormatting.LIGHT_PURPLE + StatCollector.translateToLocal("tooltip.gadget.range")
                    + ": "
                    + GadgetUtils.getToolRange(stack));
        }

        if (getToolMode(stack) == BuildingModes.Surface) list
            .add(ChatFormatting.GOLD + StatCollector.translateToLocal("tooltip.gadget.fuzzy") + ": " + getFuzzy(stack));

        addInformationRayTraceFluid(list, stack);

        list.add(
            ChatFormatting.YELLOW + StatCollector.translateToLocal("tooltip.gadget.building.place_atop")
                + ": "
    ...
    private boolean build(EntityPlayer player, ItemStack stack) {
        ...
        if (!blockState.isAir()) {
            ...
            // BlockState state = Blocks.AIR.getDefaultState(); //Initialize a new State Variable for use in the fake
            // world
    ...
    public boolean undoBuild(EntityPlayer player) {
        ...
        // List<ChunkCoordinates> failedRemovals = new ArrayList<ChunkCoordinates>(); //Build a list of removals that
        // fail
        ...
        // currentBlock.getBlock().harvestBlock(world, player, coord, currentBlock, world.getTileEntity(coord),
        // silkTool);
    ...
    private boolean placeBlock(World world, EntityPlayer player, ChunkCoordinates pos, BlockState setBlock) {
        ...
        if (ForgeEventFactory.onPlayerBlockPlace(player, blockSnapshot, ForgeDirection.UP)
            .isCanceled()) {
            return false;
        }
        ...
        if (useItemSuccess) {
            ...
            world.spawnEntityInWorld(
                new BlockBuildEntity(
                    ...
                    setBlock,
                    1,
                    GadgetUtils.getToolActualBlock(heldItem),
