...
//        registerMessage(PacketDestructionGUI.Handler.class, PacketDestructionGUI.class, Side.SERVER);
    ...
    private static void registerMessage(Class handler, Class packet, Side side) {
        if (side != Side.CLIENT) registerMessage(handler, packet, cpw.mods.fml.relauncher.Side.SERVER);

        if (side != Side.SERVER) registerMessage(handler, packet, cpw.mods.fml.relauncher.Side.CLIENT);
    }
