...
public class ReflectionTool {
     ...
     * @return A List containing the Fields declared by this class (superclasses don't count!) who match the given
     *         Predicate
     */
     ...
     * @return whether or not the instance is null if the Field is static, or non-null (and of an appropriate class) if
     *         the Field is not
     */
    public static boolean isInstanceProvidedForField(@Nonnull Field field, @Nullable Object instance) {
        return (instance != null && !ReflectionTool.PREDICATE_STATIC.test(field)
            && field.getDeclaringClass()
                .isAssignableFrom(instance.getClass()))
            || (instance == null && ReflectionTool.PREDICATE_STATIC.test(field));
    }
}
