...
public class ToolRenders {
    ...
    private static final Cache<Triple<UniqueItemStack, ChunkCoordinates, Integer>, Integer> cacheDestructionOverlay = CacheBuilder.newBuilder().maximumSize(1).expireAfterWrite(1, TimeUnit.SECONDS).removalListener(removal -> GLAllocation.deleteDisplayLists((int) removal.getValue())).build();

    // We use these as highlighters
//    private static final BlockState stainedGlassYellow = Blocks.stained_glass.getDefaultState().withProperty(COLOR, EnumDyeColor.YELLOW);
//    private static final BlockState stainedGlassRed = Blocks.STAINED_GLASS.getDefaultState().withProperty(COLOR, EnumDyeColor.RED);
//    private static final BlockState stainedGlassWhite = Blocks.STAINED_GLASS.getDefaultState().withProperty(COLOR, EnumDyeColor.WHITE);
...
//    }
    ...
    public static void renderDestructionOverlay(RenderWorldLastEvent evt, EntityPlayer player, ItemStack stack) {
        ...
        ChunkCoordinates startBlockPos = (GadgetDestruction.getAnchor(stack) == null) ? VectorTools.getPosFromMovingObjectPosition(lookingAt) : GadgetDestruction.getAnchor(stack);
        var startBlock = BlockState.getBlockState(world, startBlockPos);

        EnumFacing facing = (GadgetDestruction.getAnchorSide(stack) == null) ? EnumFacing.getFront(lookingAt.sideHit) : GadgetDestruction.getAnchorSide(stack);
        ...
        try {
            GL11.glCallList(cacheDestructionOverlay.get(new ImmutableTriple<>(new UniqueItemStack(stack), startBlockPos, facing.ordinal()), () -> {
    ...
    private static void renderDestructionOverlay(EntityPlayer player, World world, ChunkCoordinates startBlockPos, EnumFacing facing, ItemStack heldItem) {
        ...
        for (ChunkCoordinates coordinate : sortedCoordinates) {
            ...
            for (EnumFacing side : EnumFacing.values()) {
                if (state.getBlock().shouldSideBeRendered(world, coordinate.posX, coordinate.posY, coordinate.posZ, side.ordinal())) {
                    invisible = false;
                    break;
                }
            }
...
//    }
    ...
    private static void renderBox(Tessellator tessellator, double startX, double startY, double startZ, double endX, double endY, double endZ, int R, int G, int B) {
        ...
        // Set color for the first vertex (you can set transparency using alpha in this method, but here it's always 255)
    ...
    public static void renderBoundingBox(Tessellator tessellator, double minX, double minY, double minZ, double maxX, double maxY, double maxZ, float red, float green, float blue, float alpha) {
...
//            return Vec3.createVectorHelper(player.lastTickPosX + (player.posX - player.lastTickPosX) * partialTick, player.lastTickPosY + (player.posY - player.lastTickPosY) * partialTick, player.lastTickPosZ + (player.posZ - player.lastTickPosZ) * partialTick);
//        }
//
//        /**
